...
multiMiAT <- function (y, otu.tab, tree, covs = NULL, model = c("bcl", "clm"), 
  distance = c("BC", "U.UniFrac", "G.UniFrac.00", "G.UniFrac.25", 
    "G.UniFrac.50", "G.UniFrac.75", "W.UniFrac"), kernel = c("LK", 
    "GK", "LaK"), W, n.perm = 999, seed = 1) 
{
  Ks <- Whole.Kernels(otu.tab, tree, distance = distance, 
    kernel = kernel)
  if (!("bcl" %in% model) & !("clm" %in% model) & !("crm" %in% 
    model)) {
  ...
  if ("bcl" %in% model) {
    re.bcl <- multiMiRKAT_bcl(y = y, Ks = Ks, covs = covs, 
    ...
    MiRKAT.MC <- c(MiRKAT.MC, round(out.bcl$MiRKAT_MCN, 
      digits = 4))
    names(MiRKAT.MC)[length(MiRKAT.MC)] <- "MiRKAT-MCN"
  }
  if ("clm" %in% model) {
    re.clm <- multiMiRKAT_clm(y = y, Ks = Ks, covs = covs, 
      n.perm = n.perm, seed = seed)
    ...
    MiRKAT.MC <- c(MiRKAT.MC, round(out.clm$MiRKAT_MCO, 
      digits = 4))
    names(MiRKAT.MC)[length(MiRKAT.MC)] <- "MiRKAT_MCO"
  }
  if ("crm" %in% model) {
    re.crm <- multiMiRKAT_crm(y = y, Ks = Ks, covs = covs, 
      n.perm = n.perm, seed = seed)
  ...
  score.test <- as.numeric(round(score.test_bcl(y, otu.tab, 
    covs, W), digits = 4))
  names(score.test) <- "score.test"
  multiMiAT.pvs <- c(multiMiRKAT, score.test, MiRKAT.MC)
  multiMiAT.pvs[which(multiMiAT.pvs == 0)] <- 1e-06
  multiMiAT <- round(harmonicmeanp::p.hmp(p = multiMiAT.pvs, 
    w = NULL, L = length(multiMiAT.pvs), w.sum.tolerance = 1e-06, 
    multilevel = F)[1], digits = 4)
  names(multiMiAT) <- "multiMiAT"
  multiMiAT <- c(list(multiMiRKAT.pvs$multiMiRKAT.bcl.pvs), 
    list(multiMiRKAT.pvs$multiMiRKAT.clm.pvs), list(score.test), 
    list(MiRKAT.MC), list(multiMiRKAT), list(multiMiAT))
  names(multiMiAT) <- c("multiMiRKAT.bcl.pvs", "multiMiRKAT.clm.pvs", 
    "Score.test", "MiRKAT.MC", "multiMiRKAT", "multiMiAT")
