#' @export
MiRKAT_MCN <- function(y, Ks, covs = NULL){
  ...
  if (is.null(covs)) {
    ...
    xnam <- colnames(covs)
    formula <- as.formula(paste("y.factor ~ ", paste(xnam, collapse= "+")))
  }
  
  inner.GLM.null <- function(formula, data = covs){
  ...
  KY <- inner.GLM.null(formula, covs)
  ...
  p.MiRKAT_MCN <- harmonicmeanp::p.hmp(p = pvs.all, w = NULL, L = length(pvs.all), w.sum.tolerance = 1e-6, multilevel = F)[1]

  MiRKAT_MCN <- list(MiRKAT_MCN.pvs = pvs, MiRKAT_MCN = p.MiRKAT_MCN)
