#' @export
multiMiRKAT_crm <- function (y, Ks, covs = NULL, n.perm = 999, seed = 1) {
  ...
  if (is.null(covs)) {
    fit <- vglm(y.ordered ~ 1, cratio(parallel = TRUE))
    fitprob <- fitted.values(fit)
    prob.vector <- as.vector(t(fitprob[,1:(ncategories - 1)]))
  } else {
    fit <- vglm(y.ordered ~ ., data = covs, cratio(parallel = TRUE))
    fitprob <- fitted.values(fit)
    prob.vector <- as.vector(t(fitprob[,1:(ncategories - 1)]))
  }
  ...
  for (i in 1:length(Ks[[1]])) {
    ...
    multiMiRKAT.crm.pvs <- c(multiMiRKAT.crm.pvs, list(mirkat.pvs))

  }
  
  T0 <- apply(allQ0.omni, 1, min)
  T <- min(allQ.omni)
  p.multiMiRKAT.crm <- (length(which(T0 < T)) + 0.01)/(n.perm + 0.01)
  names(p.multiMiRKAT.crm) <- "multiMiRKAT.crm"
  T.statistic <- list(T = T, T0 = T0)
  names(T.statistic) <- c("T.statistic", "T0.statistic")

  multiMiRKAT.crm.pvs <- lapply(multiMiRKAT.crm.pvs, function(x) round(x, digits = 4))
  p.multiMiRKAT.crm <- round(p.multiMiRKAT.crm, digits = 4)
  names(multiMiRKAT.crm.pvs) <- names(Ks[[1]])
  
  multiMiRKAT.crm <- list(T.statistic = T.statistic, multiMiRKAT.crm.pvs = multiMiRKAT.crm.pvs, multiMiRKAT.crm.Omnibus.p = p.multiMiRKAT.crm)

  return(multiMiRKAT.crm = multiMiRKAT.crm)
